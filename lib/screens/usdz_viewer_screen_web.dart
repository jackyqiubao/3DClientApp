...
class _UsdzViewerScreenState extends State<UsdzViewerScreen> {
  ...
  void initState() {
    super.initState();
    _viewType = 'usdz-viewer-${widget.uid}-${DateTime.now().microsecondsSinceEpoch}';
    _setupHtmlView();
    _loadRemoteUsdZ();
  }
...
''',
  ...
  Future<void> _loadRemoteUsdZ() async {
    try {
      ...
      if (!mounted) return;
      _setModelSource('data:model/vnd.usdz+zip;base64,${base64Encode(bytes)}');
      setState(() {
        _loading = false;
        _error = null;
      });
      ...
      setState(() {
        _loading = false;
        _error = 'Remote USDZ load failed. Please upload a local USDZ file.';
      });
    }
  }

  void _setModelSource(String src) {
    final html.Element? viewer = _container?.querySelector('#flutter-usdz-viewer');
    if (viewer != null) {
            ...
            child: Row(
              children: <Widget>[
                ElevatedButton(
                  ...
                  child: const Text('Upload USDZ file'),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Text(
                    'You can upload a local USDZ file and preview it directly in the browser.',
